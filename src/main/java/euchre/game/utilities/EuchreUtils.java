...
public class EuchreUtils {
	...
	public static boolean compareCards(Card card1, Card card2, Suite trump, Suite leadSuite) { 
		...
		boolean condb = card1IsLeadSuite && card2IsLeadSuite
				&& card1.getValue().getValue() > card2.getValue().getValue();
		boolean cond2 = card1IsTrump && !card2IsTrump;
		boolean bothTrump = card1IsTrump && card2IsTrump;
		boolean cond5 = card1IsLeft && !card2IsRight; // card1 is left card2 is
														// right
		...
		while (notDone) {
			...
			do {
				...
				for (int i = 0; i < cards.size() - 1; i++) {
					...
					if (compareCards(card1, card2, trump, leadSuite)) {
